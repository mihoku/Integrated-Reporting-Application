...
var currentuser = _db.Users.Where(y => y.UserName == active.UserName && y.isRevoked == false).FirstOrDefault();
            ...
            var flashKegiatan2 = db.TransFlashKegiatan.Where(y => y.Finalize == status.ID && y.UnitID == currentuser.UnitID).Include(r => r.RefPegawai).Include(r => r.RefUnitPJ);
            return PartialView("_listKegiatan", flashKegiatan2.ToList());
        }
...
var currentuser = _db.Users.Where(y => y.UserName == active.UserName && y.isRevoked == false).FirstOrDefault();
            ...
            if (currentuser.RoleID == 1)
            {
                ViewBag.ManajerID = new SelectList(db.RefPegawai.Where(y => y.Aktif == true), "ID", "PegName", kegiatan.ManajerID);
                ViewBag.UnitID = new SelectList(db.RefUnitPJ.Where(y => y.Aktif == true && y.isPrimeMover == false), "ID", "Detail", kegiatan.UnitID);
                ViewBag.SysUsername = User.Identity.GetUserName();
                ViewBag.SysWorkstation = Request.ServerVariables["REMOTE_HOST"];
                ViewBag.SysTglEntry = DateTime.Now;
                ViewBag.Finalize = kegiatan.Finalize;
                return View(kegiatan);
            }

            ViewBag.ManajerID = new SelectList(db.RefPegawai.Where(y => y.Aktif == true && y.PegUnitID == currentuser.UnitID), "ID", "PegName", kegiatan.ManajerID);
            ViewBag.UnitID = new SelectList(db.RefUnitPJ.Where(y => y.Aktif == true && y.isPrimeMover == false && y.ID == currentuser.UnitID), "ID", "Detail", kegiatan.UnitID);
            ViewBag.SysUsername = User.Identity.GetUserName();
            ViewBag.SysWorkstation = Request.ServerVariables["REMOTE_HOST"];
            ViewBag.SysTglEntry = DateTime.Now;
            ViewBag.Finalize = kegiatan.Finalize;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,UnitID,Judul,ManajerID,TanggalKasus,SysUsername,SysTglEntry,SysWorkstation,FInalize")] TransFlashKegiatan flashKegiatan)
        {
                        MembershipUser active = Membership.GetUser(User.Identity.Name);
...
var currentuser = _db.Users.Where(y => y.UserName == active.UserName && y.isRevoked == false).FirstOrDefault();
            ...
            TransFlashKegiatan kegiatan = db.TransFlashKegiatan.Find(flashKegiatan.ID);
            RefPegawai pj = db.RefPegawai.Find(flashKegiatan.ManajerID);
            RefUnitPJ unit = db.RefUnitPJ.Find(flashKegiatan.UnitID);
            ...
            if (currentuser.RoleID != 1 && (pj.PegUnitID != currentuser.UnitID || unit.ID != currentuser.UnitID))
            {
                return RedirectToAction("NotFound", "Error");
            }
            ...
            if (ModelState.IsValid)
            {
                kegiatan.Judul = flashKegiatan.Judul;
                kegiatan.ManajerID = flashKegiatan.ManajerID;
                kegiatan.UnitID = flashKegiatan.UnitID;
                kegiatan.TanggalKasus = flashKegiatan.TanggalKasus;
                kegiatan.SysUsername = User.Identity.GetUserName();
                kegiatan.SysWorkstation = Request.ServerVariables["REMOTE_HOST"];
                kegiatan.SysTglEntry = DateTime.Now;
                db.SaveChanges();
                return RedirectToAction("Details", "Flash", new { id = flashKegiatan.ID });
            }
            if (currentuser.RoleID == 1)
        ...
        public ActionResult Progress(int id)
        {
            ...
            TransNDPermintaanFlash nd = db.TransNDPermintaanFlash.Where(y => y.Tahun == flash.Tahun && y.PeriodeID == flash.Period).OrderByDescending(y => y.ID).FirstOrDefault();
